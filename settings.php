...
if($t == "news") :   
    ...
    ?>
        <div class="row mt-4">
            ...
            <table class="table table-striped table-hover border border-5 table-bordered">
                <thead>
                    <tr>
                        <th>Действия</th>
                        <th>id</th>
                ...
                <tbody>
                    ...
                    <tr>
                        <td>
                            <a href="update.php?id=<?= $row["id"] ?>" class="btn btn-primary">Изменить</a>
...
<?php if($t == "prizes") :
    $sql = "SELECT *, promos1.promo_name FROM prizes 
    LEFT JOIN promos1 ON (prizes.promo_id = promos1.id) ORDER BY promos1.promo_name";
    $result = $pdo->query($sql);
    ?>
        <div class="row mt-4">
            <div class="col table-responsive">
...
<?php if($t == "requests") :
    $sql = "SELECT requests.id, promos.promo_name, prizes.prize_name, users.login FROM requests 
...
<?php if($t == "codes") : 
    ...
    $result = $pdo->query($sql);
    ?>
        <form action="action/codegen-action.php" method="post" class="my-2" enctype="multipart/form-data">
